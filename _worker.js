...
let dohURL = 'https:///dns-query'; // https://cloudflare-dns.com/dns-query or https://dns.google/dns-query
	...
	userIDArray.forEach((userID) => {
		...
		if (!hostName.includes('pages.dev')) {
			...
			portArray_http.forEach((port) => {
				const commonUrlPart_http = `:${port}?encryption=none&security=none&fp=&type=ws&host=${hostName}&path=%2F#${hostName}-HTTP-${port}`;
		...
		portArray_https.forEach((port) => {
			const commonUrlPart_https = `:${port}?encryption=none&security=tls&sni=${hostName}&fp=&type=ws&host=${hostName}&path=%2F#${hostName}-HTTPS-${port}`;
